...
@pytest.fixture
def windsurfing(page: Page):
    return WindSurfing(page)

def test_click_egipt_button(page: Page, windsurfing: WindSurfing):
    
    start_page = StartPage(page)
    page.goto("https://fakestore.testelka.pl/")
    start_page.click_windsurfing()
    windsurfing.click_egipt()
    expect(page).to_have_url("https://fakestore.testelka.pl/product/egipt-el-gouna/")

def test_add_egipt_to_cart(page: Page, windsurfing: WindSurfing):
